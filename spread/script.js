...
(function() {
        ...
        reader.onload = function(readerEvt) {
          ...
          img.onload = function(imageEvt) {
            canvas.dispatchEvent(
              new CustomEvent('imagedrop', {
                detail: imageEvt.target
              }));
          }
  ...
  it.reset = function(opts) {
    ...
    rf.dragRegistry.add(renderer, null /* bounds */ , firstDrop, colorDrop, function() {});
    ...
    if (environment.walls) {
      ...
      for (var y = 0; y < environment.modelHeight; y++) {
        ...
        for (var add = 0; add <= x - lastX; add++) {
          it.driver.createOrGetCell(x + add, y)
            .isWall = true;
          it.driver.createOrGetCell(environment.modelWidth - x - add, y)
            .isWall = true;
        }
        lastX = x;
        wallX += wallStep;
  ...
  it.Driver.prototype.imageDrop = function(image) {
    ...
    for (var i = 0; i < this.env.modelWidth; i++) {
      for (var j = 0; j < this.env.modelHeight; j++) {
        ...
        cell.x = i;
        cell.y = j;
        var rgb = context.getImageData(i, j, 1, 1)
          .data;
  ...
  class SpreadWrapperPage extends Page {
    constructor() {
      super('spread');
    }
    getContainer() {
  ...
  rf.pageViewer.registerPage(new SpreadWrapperPage());
